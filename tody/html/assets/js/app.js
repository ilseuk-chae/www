!(function () {
    ...
    //  */
    // function E() {
    //     // p 함수를 호출하여 초기 언어를 설정합니다. y가 null이면 b(kr)를 사용하고, y가 null이 아니면 y를 사용합니다.
    //     p(null === y ? b : y);
    //     // 클래스 이름이 "language"인 모든 요소를 가져옵니다.
    //     var e = document.getElementsByClassName("language");
    //     // 만약 클래스 이름이 "language"인 요소들이 존재하면
    //     e &&
    //         // 이 요소들을 배열로 변환하여 각각에 대해 다음 함수를 실행합니다.
    //         Array.from(e).forEach(function (t) {
    //             // 각 요소에 클릭 이벤트 리스너를 추가합니다.
    //             t.addEventListener("click", function (e) {
    //                 // 요소가 클릭되면 해당 요소의 "data-lang" 속성 값을 가져와서 p 함수를 호출합니다.
    ...
    /**
     * 선택된 언어에 맞는 플래그 이미지를 변경합니다.
     * 선택된 언어를 로컬 스토리지에 저장합니다.
     * 언어 JSON 파일을 불러와서 페이지의 텍스트를 해당 언어로 업데이트합니다.
     * @param {*} e = language
     */
    function p(e) {
        // "header-lang-img" 요소가 존재하는지 확인하고, 존재하면 아래 조건문을 실행합니다.
        document.getElementById("header-lang-img") &&
            // 언어 코드에 따라 "header-lang-img" 요소의 src 속성을 설정합니다.
            ("kr" == e
                ? (document.getElementById("header-lang-img").src = "/assets/images/flags/kr.svg")
                : "en" == e
            ...
            // 로컬 스토리지에서 언어를 가져와 변수 y에 할당합니다. 만약 null이면, 다시 p(b)를 호출하여 기본 언어로 설정합니다.
            null == (y = localStorage.getItem("language")) && p(b),
            // XMLHttpRequest 객체를 생성하여 GET 요청을 통해 해당 언어의 JSON 파일을 가져옵니다.
            (e = new XMLHttpRequest()).open("GET", "/assets/lang/" + y + ".json"),
            (e.onreadystatechange = function () {
                var a;
                // 요청의 상태가 완료되고(readyState 4), 응답이 성공적(HTTP 상태 200)이면
                4 === this.readyState &&
                    200 === this.status &&
                    ...
                    Object.keys(a).forEach(function (t) {
                        ...
                        // 각 요소의 텍스트 콘텐츠를 JSON 객체의 값으로 설정합니다.
    ...
    /**
     * 내비게이션 바의 Collapse 요소들을 관리
     * 하나의 메뉴가 열리면 다른 메뉴는 자동으로 닫히도록 한다.
     * 각 Collapse 요소에 대해 Bootstrap의 Collapse 인스턴스를 생성
     * 열릴 때 다른 Collapse 요소들을 닫고, 닫힐 때 하위 Collapse 요소들도 함께 닫도록 설정
     */
    function f() {
        var e;
        // .navbar-nav 안의 모든 .collapse 요소를 선택
        document.querySelectorAll(".navbar-nav .collapse") &&
            ((e = document.querySelectorAll(".navbar-nav .collapse")),
            Array.from(e).forEach(function (t) {
                // 각 Collapse 요소 t에 대해 Bootstrap의 Collapse 인스턴스를 생성
                // toggle: !1 옵션을 사용하여 초기 상태에서는 자동으로 열리지 않게한다.
                var a = new bootstrap.Collapse(t, { toggle: !1 });

                t.addEventListener("show.bs.collapse", function (e) {
                    // Collapse 요소가 열릴 때(show.bs.collapse 이벤트), 이벤트 전파를 막음
                    e.stopPropagation();

                    // 현재 열리는 Collapse의 부모 요소 중 가장 가까운 Collapse 요소를 찾아서,
                    var e = t.parentElement.closest(".collapse");

                    e
                        ? // 그 안의 다른 모든 Collapse 요소들을 닫는다.
                          ((e = e.querySelectorAll(".collapse")),
                          Array.from(e).forEach(function (e) {
                              e = bootstrap.Collapse.getInstance(e);
                              e !== a && e.hide();
                          }))
                        : // 만약 부모 요소에 Collapse가 없으면
                          ((e = (function (e) {
                              // 현재 요소의 부모의 형제 요소들을 찾아서 처리
                              for (var t = [], a = e.parentNode.firstChild; a; ) 1 === a.nodeType && a !== e && t.push(a), (a = a.nextSibling);

                              return t;
                          })(t.parentElement)),
                          Array.from(e).forEach(function (e) {
                              // 각 형제 요소에 대해 여러 작업을 수행하여, 다른 Collapse 요소들을 닫고 aria-expanded 속성을 false로 설정
                    ...
                    // Collapse 요소가 닫힐 때(hide.bs.collapse 이벤트), 이벤트 전파를 막고, 하위에 있는 모든 Collapse 요소들을
    ...
    /**
     * 웹페이지의 내비게이션 메뉴를 설정하고,
     * 특정 조건에 따라 메뉴의 스타일과 동작을 변경한다.
     */
    function v() {
        var o,
    ...
    /**
     * 주어진 HTML 요소 e가 현재 뷰포트(사용자가 보고 있는 화면 영역) 내에 완전히 포함되어 있는지를 확인
     * @param {*} e
     * @returns
     */
    function h(e) {
    ...
    /**
     * 웹페이지의 레이아웃 설정에 따라 특정 요소의 상태와 속성을 조정
     */
    function S() {
    ...
    /**
     * 브라우저 창의 너비에 따라 웹페이지의 레이아웃과 스타일을 동적으로 조정
     */
    function I() {
        feather.replace(); // feather 아이콘을 SVG로 대체

        // 브라우저 창의 너비에 따라 레이아웃 설정
        var e = document.documentElement.clientWidth,
            e =
                (e < 1025 && 767 < e
                    ? (document.body.classList.remove("twocolumn-panel"),
                      "twocolumn" == sessionStorage.getItem("data-layout") && (document.documentElement.setAttribute("data-layout", "twocolumn"), document.getElementById("customizer-layout03") && document.getElementById("customizer-layout03").click(), v(), A(), f()),
    ...
    /**
     * 네비게이션 메뉴의 드롭다운 위치를 조정하여 화면 오른쪽에 충분한 공간이 있는지 확인하고, 필요에 따라 스타일을 변경
     * @param {*} e
     */
    function w(e) {
    ...
    /**
     * 웹 페이지가 로드된 후, 화면 크기가 변경될 때, 스크롤이 발생할 때, 그리고 특정 요소가 클릭될 때 다양한 동작을 처리
     * 전체 화면 모드 토글, 다크 모드 토글, 코드 보기 전환, 스크롤 시 상단 바 그림자 추가, 사이드바 토글 등 다양한 기능을 포함
     */
    function O() {
        // DOMContentLoaded 이벤트가 발생하면 실행
        // •	code-switcher 클래스를 가진 요소들에 대해 이벤트 리스너를 추가합니다.
        // •	체크박스가 변경될 때마다 코드 보기와 라이브 프리뷰를 전환합니다.
        // •	feather.replace()는 Feather Icons를 초기화하거나 업데이트합니다.
        document.addEventListener("DOMContentLoaded", function () {
            ...
            // resize 이벤트 리스너:
            // •	윈도우 크기가 변경될 때 I 함수를 호출합니다.
            window.addEventListener("resize", I),
            // 초기 I 함수 호출:
            // •	페이지가 로드될 때 I 함수를 즉시 호출합니다.
            I(),
            // Waves.js 초기화:
            // •	Waves.init()를 호출하여 Waves 효과를 초기화합니다.
            Waves.init(),
            // scroll 이벤트 리스너:
            // •	스크롤이 발생할 때 page-topbar 요소에 topbar-shadow 클래스를 추가하거나 제거합니다.
            // •	스크롤 위치가 50px 이상이면 그림자를 추가하고, 그렇지 않으면 제거합니다.
            ...
            // 	load 이벤트 리스너:
            // •	페이지가 완전히 로드되면 호출됩니다.
            // •	페이지 레이아웃을 확인하고, twocolumn 레이아웃이면 A 함수를 호출하고, 그렇지 않으면 L 함수를 호출합니다.
            // •	vertical-overlay 클래스를 가진 요소들에 클릭 이벤트 리스너를 추가하여 클릭 시 사이드바를 숨깁니다.
            // •	z() 함수를 호출합니다.
            ...
            // 햄버거 아이콘 클릭 이벤트 리스너:
            // •	topnav-hamburger-icon 요소가 존재하면 클릭 이벤트 리스너를 추가하여 M 함수를 호출합니다.
            document.getElementById("topnav-hamburger-icon") && document.getElementById("topnav-hamburger-icon").addEventListener("click", M);

        // 화면 크기 및 레이아웃 설정:
        // •	defaultAttribute를 세션 스토리지에서 가져와 파싱합니다.
        // •	화면 너비가 767px 미만이고, 레이아웃이 twocolumn이면 two-column-menu의 모든 li 요소에 클릭 이벤트 리스너를 추가하여 twocolumn-panel 클래스를 제거합니다.
    ...
    /**
     * 웹페이지의 내비게이션 메뉴 항목을 현재 페이지에 맞게 활성화 상태로 설정하고, 레이아웃을 조정
     */
    function A() {
        feather.replace();
        var e,
    ...
    /**
     *  vertical 상태일 때, 현재 페이지에 해당하는 내비게이션 메뉴 항목을 활성화하고, 관련된 콜랩스 메뉴를 펼친다.
     */
    function L() {
        console.log("start L()");
        ...
        (e = e.substring(e.lastIndexOf("/") + 1)) &&
            ...
            (e.classList.add("show"), e.parentElement.children[0].classList.add("active"), e.parentElement.children[0].setAttribute("aria-expanded", "true"), ((e = e.children[0]), e.parentElement.closest(".collapse.menu-dropdown"))) &&
            (e.parentElement.closest(".collapse").classList.add("show"),
            e.parentElement.closest(".collapse").previousElementSibling && e.parentElement.closest(".collapse").previousElementSibling.classList.add("active"),
            e.parentElement.parentElement.parentElement.parentElement.closest(".collapse.menu-dropdown")) &&
            (e.parentElement.parentElement.parentElement.parentElement.closest(".collapse").classList.add("show"), e.parentElement.parentElement.parentElement.parentElement.closest(".collapse").previousElementSibling) &&
    ...
    // function L() {
    //     var e = "/" == location.pathname ? "index.html" : location.pathname.substring(1);
    //     console.log("Initial pathname:", e);

    //     e = e.substring(e.lastIndexOf("/") + 1);
    //     console.log("Processed pathname:", e);

    //     if (e) {
    //         e = document.getElementById("navbar-nav").querySelector('[href="' + e + '"]');
    //         console.log("Navbar link:", e);

    //         if (e) {
    //             e.classList.add("active");
    //             e = e.closest(".collapse.menu-dropdown");
    //             console.log("Closest collapse menu-dropdown:", e);

    //             if (e) {
    //                 e.classList.add("show");
    //                 console.log("Show class added:", e);
    //                 e.parentElement.children[0].classList.add("active");
    //                 e.parentElement.children[0].setAttribute("aria-expanded", "true");

    //                 var parentCollapseMenu = e.closest(".collapse.menu-dropdown");
    //                 if (parentCollapseMenu) {
    //                     parentCollapseMenu.classList.add("show");
    //                     console.log("Parent collapse menu-dropdown shown:", parentCollapseMenu);

    //                     var parentCollapse = parentCollapseMenu.closest(".collapse");
    //                     if (parentCollapse) {
    //                         var previousElement = parentCollapse.previousElementSibling;
    //                         if (previousElement) {
    //                             previousElement.classList.add("active");
    //                             console.log("Previous sibling element activated:", previousElement);
    //                         }
    //                     }

    //                     var grandParentCollapseMenu = parentCollapseMenu.closest(".collapse.menu-dropdown");
    //                     if (grandParentCollapseMenu) {
    //                         grandParentCollapseMenu.classList.add("show");
    //                         console.log("Grandparent collapse menu-dropdown shown:", grandParentCollapseMenu);

    //                         var grandParentCollapse = grandParentCollapseMenu.closest(".collapse");
    //                         if (grandParentCollapse) {
    //                             var grandPreviousElement = grandParentCollapse.previousElementSibling;
    //                             if (grandPreviousElement) {
    //                                 grandPreviousElement.classList.add("active");
    //                                 console.log("Grandparent previous sibling element activated:", grandPreviousElement);
    //                             }
    //                         }

    //                         if ("horizontal" == document.documentElement.getAttribute("data-layout")) {
    //                             var topAncestorCollapse =
    //                                 grandParentCollapseMenu.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.closest(".collapse");
    //                             if (topAncestorCollapse) {
    //                                 var topPreviousElement = topAncestorCollapse.previousElementSibling;
    //                                 if (topPreviousElement) {
    //                                     topPreviousElement.classList.add("active");
    //                                     console.log("Top ancestor previous sibling element activated:", topPreviousElement);
    //                                 }
    //                             }
    //                         }
    //                     }
    ...
    /**
     * counter-value 클래스를 가진 요소들의 숫자가 점진적으로 증가하여 목표 숫자에 도달하는 애니메이션을 적용
     * 숫자는 천 단위로 콤마가 포함된 형식으로 표시
     * 이를 통해 웹 페이지의 통계나 수치를 시각적으로 효과적으로 표현
     */
    function D() {
        // counter-value 클래스를 가진 모든 요소를 선택
        ...
        // counter-value 요소가 존재할 때
        e &&
            Array.from(e).forEach(function (n) {
                // 즉시 실행 함수로 카운터 애니메이션을 시작
                !(function e() {
                    var t = +n.getAttribute("data-target"), // 목표 숫자
                        a = +n.innerText, // 현재 숫자
                        o = t / 250; // 증가량 계산

                    // 증가량이 1보다 작으면 1로 설정
                    o < 1 && (o = 1),
                        // 현재 숫자가 목표 숫자보다 작으면 숫자를 증가시키고 재귀적으로 함수 호출
                        a < t
                            ? ((n.innerText = (a + o).toFixed(0)), // 숫자 증가
                              setTimeout(e, 1)) //1ms 후에 다시 함수 호출
                            : (n.innerText = s(t)), // 목표 숫자에 도달하면 최종 숫자 설정
                        // 현재 숫자에 천 단위 콤마 추가
                        s(n.innerText);
                })();
            });
                ...
                t.addEventListener("change", function () {
                    ...
                    document.documentElement.setAttribute(a, t.value),
                        ...
                        "data-layout-width" == a && "boxed" == t.value
                            ...
                            : "data-layout-width" == a && "fluid" == t.value && (document.documentElement.setAttribute("data-sidebar-size", "lg"), sessionStorage.setItem("data-sidebar-size", "lg"), (document.getElementById("sidebar-size-default").checked = !0)),
                        "data-layout" == a &&
                            ("vertical" == t.value
    ...
    // 1. 레이아웃 설정 로드
    // 이 부분은 sessionStorage에서 저장된 레이아웃 설정을 가져와서 적용합니다. 만약 저장된 설정이 없으면 현재 HTML의 속성을 sessionStorage에 저장합니다.
    sessionStorage.getItem("defaultAttribute")
        ...
        // 3. 전체 화면 모드 토글
        // 전체 화면 모드를 토글하는 기능을 구현합니다. 사용자가 전체 화면 버튼을 클릭하면 전체 화면 모드로 전환하거나 해제합니다.
        ...
        // 4. 다크 모드 토글
        // 버튼을 클릭하면 data-layout-mode 속성을 확인하여 다크 모드와 라이트 모드를 토글합니다.
        ...
        (m = document.querySelectorAll(".light-dark-mode")) &&
            ...
            m[0].addEventListener("click", function (e) {
                ...
                l.hasAttribute("data-sidebar") && "dark" == l.getAttribute("data-sidebar") ? C("data-sidebar", "light", "layout-sidebar", l) : C("data-sidebar", "dark", "layout-sidebar", l);
                l.hasAttribute("data-topbar") && "dark" == l.getAttribute("data-topbar") ? C("data-topbar", "light", "layout-topbar", l) : C("data-topbar", "dark", "layout-topbar", l);
            }),
        O(),
        D(),
        S(),
        ...
        document.getElementsByClassName("dropdown-item-cart") &&
            ...
            F()),
        // 6. 알림 기능 초기화
        // 알림 아이템을 체크하거나 해제할 때 발생하는 이벤트를 처리합니다. 또한, 알림 모달에서 알림을 삭제하는 기능을 구현합니다.
        ...
        // 7. 툴팁 및 팝오버 초기화
        // 모든 툴팁과 팝오버를 초기화하여 사용할 수 있게 만듭니다.
        ...
        // 10. Chices.js 초기화
        // data-choices 속성이 있는 모든 요소를 선택하여 초기화합니다.
        // 검색, 정렬, 아이템 제거 버튼 등 다양한 설정을 적용합니다.
        ...
        // 11. Flatpickr 초기화
        // data-provider 속성이 있는 모든 요소를 선택하여 초기화합니다.
        // 날짜, 시간, 날짜/시간 선택 등 다양한 설정을 적용합니다.
        ...
        // 12. 탭 초기화
        // data-bs-toggle="tab" 속성을 가진 드롭다운 메뉴의 링크를 클릭하면 해당 탭을 보여줍니다.
        ...
        // 13. 기타 초기화 함수 호출
        // resize 이벤트 리스너를 등록하여, 윈도우 크기가 변경될 때 G 함수를 호출합니다. 이를 통해 레이아웃을 재조정할 수 있습니다.
        // E(),
        f(),
        x(),
