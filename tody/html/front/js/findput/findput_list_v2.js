$(document).ready(function () {
    ...
    $("#results_table_container").on("click", "tr", function () {
        ...
        if (itemId && itemType) {
            if (itemType === 'put') {
                location.href = `../put/put_view_v2.html?viewNo=${itemId}`;
            } else if (itemType === 'find') {
                location.href = `../find/find_view_v2.html?viewNo=${itemId}`;
            } else {
...
async function loadListData(type) {
    ...
    if (type === "put") {
        registerBtn.attr("href", "../put/put_write_v2.html");
        registerBtnText.text("팝니다 등록");
    } else { // type === "find"
        registerBtn.attr("href", "../find/find_write_v2.html");
    ...
    const apiEndpoint = (type === "put") ? "/front/back/put/put_list_v2.php" : "/front/back/find/find_list_v2.php";
    ...
    if (type === "put") {
        columns = [
            ...
            { data: "estate_type", title: "매물종류", render: (data, type, row) => (row.sub_estate && row.sub_estate !== data) ? `${data}(${row.sub_estate})` : data },
