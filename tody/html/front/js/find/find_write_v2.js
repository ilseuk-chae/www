...
$(document).ready(function () {
    if (!userInfo()) {
        ...
        location.href = "../findput/findput_list_v2.html";
        return;
    }
    initModal();
    initSelect();
...
function findWrite() {
    ...
    callApiAbort("/front/back/find/find_write_v2.php", "POST", dataObj, "findWrite")
    //callApiFormData("POST", "/front/back/find/find_write_v2.php", formData)
        .then((response) => {
            if (!response) {
            ...
            if (statusCode == 200 && message == "SUCCESS") {
                ...
                $("#modalCompletion").on("closed", function () {
                    console.log("Modal closed");
                    location.href = "/front/views/mypage/mypage_find_v2.html"; // 페이지 리로드
                });
            } else {
