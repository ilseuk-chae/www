let subEstateMap = {}; // estate_type_code → [{type_code, type_name}]
...
async function estate_type_get() {
    ...
    callApiAbort("/front/back/find/estate_type_get_v2.php", "POST", dataObj, "estate_type_get")
        .then((response) => {
            ...
            // estate_type 변경 시 sub_estate 드롭다운 갱신
...
function initDetail() {
    ...
    callApiAbort("/front/back/find/find_detail_v2.php", "POST", dataObj, "initDetail")
        .then(async (response) => {
            ...
            // sub_estate 설정: estate_type 선택 후 옵션 갱신 및 기존 값 선택
            if (data.estate_type_cd) {
                updateSubEstateOptions(data.estate_type_cd);
                if (data.sub_estate) {
                    $("#sub_estate").val(data.sub_estate);
                }
            }
...
function findModify() {
    ...
    const dataObj = {
        ...
        sub_estate: encodeURIComponent($("#sub_estate").val() || ""),
    ...
    callApiAbort("/front/back/find/find_modify_v2.php", "POST", dataObj, "findModify")
        .then((response) => {
            // API 호출 성공
            if (!response) {
...
async function findDelete() {
    ...
    const result = await callApi("POST", "/front/back/mypage/find_delete_v2.php", dataObj);
    ...
    if (message === "SUCCESS") {
        ...
        if (confirm) location.href = "/front/views/mypage/mypage_find_v2.html";
    } else {
