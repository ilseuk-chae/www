...
async function user_info() {
    ...
    const result = await callApi("POST", "/front/back/mypage/user_info_realtor.php", dataObj);
    ...
    const { id, agency_name, registered_broker_name, zipcode, address_primary, address_detail, email, mobile, phone, homepage_url, business_regist_code, brokerage_cert_name, brokerage_cert_url, business_license_code, business_license_name, business_license_url, event_rcv_fg, term_fg, role } =
        responseData;
    if (role != "002") location.href = "mypage_v2.html";
    const [emailPart, domainPart] = email.split("@");

    $("#email").val(emailPart);
    $("#domain").val(domainPart);
    $("#phone").val(phoneOnDash(phone));
    $("#mobile").val(phoneOnDash(mobile));

    // 사업자등록증
    const businessResult = await user_image("business");
    if (businessResult) {
        if (businessResult.file_name) {
            $("#business_license_name").val(businessResult.file_name);
        }
        if (businessResult.token) {
            $("#show_business").attr("href", `/front/back/mypage/user_image.php?token=${encodeURIComponent(businessResult.token)}&type=${encodeURIComponent("business")}`);
        }
    }

    // 중개업등록증
    const brokerageResult = await user_image("brokerage");
    if (brokerageResult) {
        if (brokerageResult.file_name) {
            $("#brokerage_cert_name").val(brokerageResult.file_name);
        }
        if (brokerageResult.token) {
            $("#show_brokerage").attr("href", `/front/back/mypage/user_image.php?token=${encodeURIComponent(brokerageResult.token)}&type=${encodeURIComponent("brokerage")}`);
        }
    }

    // 이벤트 수신 동의 여부
    if (event_rcv_fg == "Y") {
        $("#event_rcv_fg").prop("checked", true);
    } else {
        $("#event_rcv_fg").prop("checked", false);
    }
...
async function user_image(rcvType) {
    const dataObj = {
        ...userInfo(),
        type: rcvType,
    };

    const result = await callApi("POST", "/front/back/mypage/user_image_token.php", dataObj);
...
function initEvents() {
    ...
    $("#modify_ready_btn").on("click", function () {
        ...
        $("form input").not(":lt(5)").not("#business_license_name, #brokerage_cert_name").prop("disabled", false);
...
function kakaoUnlinkAndWithdrawal() {
    ...
    // 인증 URL 생성
    const authUrl = `https://kauth.kakao.com/oauth/authorize?response_type=code&client_id=${REST_API_KEY}&redirect_uri=${encodeURIComponent(REDIRECT_URI)}&prompt="none"&state=${state}`;
...
async function user_info_modify() {
    try {
        ...
        const phone = form.find("#phone").val().replace(/-/g, "");
        const homepage_url = form.find("#homepage_url").val();
        ...
        const result = await callApi("POST", "/front/back/mypage/modify_realtor.php", dataObj);
        const { statusCode, message } = result;
