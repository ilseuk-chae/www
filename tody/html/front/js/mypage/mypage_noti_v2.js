...
async function notification_get() {
    ...
    const result = await callApi("POST", "/front/back/mypage/notification_get_v2.php", dataObj);
    if (!result) return;
...
async function estate_type_get() {
    const dataObj = {};
    const result = await callApi("POST", "/front/back/find/estate_type_get_v2.php", dataObj);
    if (!result) return;
...
async function noti_save(user) {
    try {
        ...
        const result = await callApi("POST", "/front/back/mypage/notification_save_v2.php", dataObj);
