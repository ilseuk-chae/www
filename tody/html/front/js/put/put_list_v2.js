$(document).ready(function () {
    ...
    $("#put_results_table_container").on("click", "tr", function () {
        ...
        if (putNo) {
            location.href = `put_view_v2.html?viewNo=${putNo}`;
        }
    });
...
function initializeDataTable() {
    ...
    const table = new DataTable("#put_results_table", {
        ...
        ajax: function (data, callback, settings) {
            ...
            callApi("POST", "/front/back/put/put_list_v2.php", requestParams) // 병합된 파라미터 전달
                .then((response) => {
...
function initFindList() {
    ...
    callApiAbort("/front/back/put/put_list_v2.php", "POST", dataObj, "loadLists")
        .then((response) => {
            // API 호출 성공
            ...
            if (responseData.length > 0) {
                const liHtml = responseData
                    .map(function (item) {
                        ...
                        return `
                            <li onclick="location.href='put_view_v2.html?viewNo=${item.put_no}';">
