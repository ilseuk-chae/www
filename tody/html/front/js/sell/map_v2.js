let mapContainer = document.getElementById("map_bg"); // 지도를 표시할 div
...
let currentUnit = "pyeong"; // 현재 단위를 추적 (기본: 평)
...
/**
 * 매물종류(estate_type)별 기본색/연한색 반환
 * @param {string} estateType - 매물종류 텍스트 (아파트, 연립/다세대 등)
...
/**
 * 매물종류 + 세부종류(sub_estate)로 마커 표시명 반환
 * @param {string} estateType - 매물종류 (아파트, 상업/업무용 등)
 * @param {string} subEstate  - 세부종류 코드 (01, 02, 03 등)
 * @returns {string} 표시명
 */
function getEstateDisplayName(estateType, subEstate) {
    // sub_estate 코드는 구 데이터("01","02","03") 와 신규 데이터("1001-02","1006-03" 등) 두 형식이 혼재
    if (estateType === "아파트") {
        // 분양권: 신규='1001-02'
...
function addContentsMarker(data, estateTypeClass) {
    ...
    switch (data.sale_type) {
        case "임대":
            priceHtml = `${formatPrice(data.rent_price, "only-uk", true)}`;
            break;
        case "매매":
            priceHtml = `${formatPrice(data.sale_price, "only-uk", true)}`;
            break;
        case "교환":
            priceHtml = `${formatPrice(data.sale_price, "only-uk", true)}`;
            break;
    ...
    iwContent.style.cssText = "padding:5px 5px 0 5px; display:inline-flex; flex-direction:column; align-items:center;";
    iwContent.innerHTML = `
        <ul class="text-center bg-white overflow-hidden" style="min-width:60px; border-radius:10px; border: 2px solid #dc3545; list-style:none; margin:0; padding:0;">
            <li style="background:linear-gradient(to right, ${lightColor}, ${baseColor}); padding:4px 7px; display:flex; align-items:center; justify-content:center; gap:3px; border-bottom: 1px solid ${baseColor}; white-space:nowrap;">
                <span style="display:inline-flex; align-items:center; justify-content:center; width:15px; height:15px; border-radius:50%; background:${baseColor}; font-size:8px; font-weight:800; color:#fff; flex-shrink:0; line-height:1; border:1px solid rgba(255,255,255,0.6);">매</span>
                <span style="font-size:10px; font-weight:700; color:#222;">${getEstateDisplayName(data.estate_type, data.sub_estate)}</span>
            </li>
            ...
            <li class="sell-area-toggle" data-raw-area="${rawArea || ''}" style="background:${baseColor}; padding:3px 8px; color:#fff; font-size:12px; cursor:pointer;" title="클릭하여 단위 변환">
                ${areaHtml}
            </li>
        </ul>
        <div style="width:100%; text-align:center; line-height:0; margin:0; padding:0;">
            <span style="display:inline-block; width:0; height:0; border-left:9px solid transparent; border-right:9px solid transparent; border-top:10px solid ${baseColor};"></span>
        </div>
    `;

    // 면적 단위 토글 (클릭 시 평 ↔ ㎡)
    const areaToggleEl = iwContent.querySelector(".sell-area-toggle");
    ...
    var customOverlay = new kakao.maps.CustomOverlay({
        ...
        position: latlng,
        xAnchor: 0.5,
        yAnchor: 1.2,   // 삼각형 끝이 좌표를 가리키도록 (실거래가와 동일)
        zIndex: 1,
    });
