...
$(document).ready(async function () {
    ...
    // → 500ms 후 relayout() + estateNewList() 강제 호출로 보정
    if ($(window).width() <= 991) {
        setTimeout(function() {
            if (typeof map !== 'undefined' && map) {
                map.relayout();
                if (typeof estateNewList === 'function') {
                    estateNewList();
                }
            }
        }, 500);
...
function initAction() {
    ...
    $("#mapContentOpenClose").click(function () {
        ...
        $(".map-sale-group").toggleClass("active");
        $(".map-estate-group").toggleClass("active");
        $(".map-exchange-group").toggleClass("active");
        $(".map-urgentsale-group").toggleClass("active");
        $(".map-price-group").toggleClass("active");
        //$(".map-sell-view").toggleClass("active");
...
async function estateList(searchNo = "", propertyNo = "") {
    ...
    callApiAbort("/front/back/sell/estate_list_v2.php", "POST", dataObj, "estateList")
        .then((response) => {
            if (!response) return;
            ...
            if (!Array.isArray(responseData) || responseData.length === 0) {
                ...
                liHtml = responseData.map(function (data, index) {
                    ...
                    if (zoomLevel > 4) {
                        // 줌레벨 5 이상: 원안에 개수 표시
                        const clusterer = createClustererAll("all");
                        const marker = createClusteredMarker(data);
                        clusterer.addMarker(marker);
                    /*} else if (zoomLevel === 4) {
                        // 줌레벨 4: 점표시 (타입별 색상 + 빨간 테두리) — 추후 활성화 예정
                        const dotEl = document.createElement("div");
                        const dotColor = getEstateTypeColors(data.estate_type).base;
                        dotEl.style.cssText = `width:10px; height:10px; border-radius:50%; background:${dotColor}; border:2px solid #dc3545; cursor:pointer;`;
                        ...
                        sellDotOverlays.push(dotOverlay);*/
                    } else {
                        // 줌레벨 4 이하: 타입별 클러스터러 → 풀마커 표시
                        const clusterer = createClusterer(data.estate_type, data.sale_type);
                        const marker = createClusteredMarker(data);
                        clusterer.addMarker(marker);
                    }
            ...
            li.children("dl")
                ...
                .done(function () {
                    ...
                    li.children("dl")
                        ...
                        .promise()
                        .done(function () {
                            updateCompareList(); // 재렌더링 후 비교 상태 복원
                        });
                });
        })
        .catch((error) => {
        ...
        .then((response) => {
            ...
            if (!Array.isArray(responseData) || responseData.length === 0) {
                ...
                liHtml = responseData.map(function (data, index) {
                    ...
                    return `
                        <dl class="estate-card ${estateTypeClass} d-flex flex-wrap" data-estate-no="${data.estate_no}">
...
function updateCompareList(){
    ...
    $('.mcs-list dl.estate-card').each(function() {
    ...
    compareList.data.forEach(compareItem => {
        ...
        if (currentDisplayedEstateNos.has(estateNo)) {
            ...
            $(`.mcs-list dl.estate-card[data-estate-no="${estateNo}"]`).addClass('dl-highlight-border');
...
async function estateDetail(estateNo) {
    ...
    const url = "/front/back/sell/estate_detail_v2.php";
    const dataObj = {
        estate_no: estateNo,
    };
...
function renderComparisonTable(comparisonData) {
    ...
    for (let i = 0; i < comparisonData.length; i++) {
        ...
        const estate_html = itemData.estate_type || "";
        // 내용 채우기
...
function createClusteredMarker(data) {
    ...
    marker.sub_estate  = data.sub_estate; // 세부 매물종류
    marker.tot_area    = data.totArea;    // 면적 (토지 외)
    marker.plat_area   = data.platArea;   // 대지면적 (토지)
    ...
    // // Create the custom overlay content (cluster design) - v2: 매물종류별 색상 적용
    // const _colors = getEstateTypeColors(data.estate_type);
    // const _base = _colors.base;
    // const _light = _colors.light;
    // const _area = (data.estate_type === "토지") ? formatArea(data.platArea) : formatArea(data.totArea);
    // var clusterContent = `
    //     <ul class="text-center bg-white overflow-hidden" style="min-width:60px; border-radius:10px; border: 1px solid ${_light};">
    //         <li class="up bg-white p-1" style="line-height:11px; display:flex; align-items:center; justify-content:center; gap:3px; border-bottom: 1px solid ${_light};">
    //             <span style="display:inline-flex; align-items:center; justify-content:center; width:14px; height:14px; border-radius:50%; background:${_base}; font-size:8px; font-weight:800; color:#fff; flex-shrink:0; line-height:1;">매</span>
    //             <span class="font10" style="color:#222;">${data.estate_type}</span>
    //         </li>
    //         <li class="up bg-white p-1">
    //             <p class="font12" style="line-height:12px;">${priceHtml}</p>
    //         </li>
    //         <li class="text-white p-1" style="background:${_light};">
    //             <span class="font11">${_area}</span>
    //         </li>
    //     </ul>
    //     <div style="text-align:center; line-height:0; margin:0; padding:0;"><span style="display:inline-block; width:0; height:0; border-left:9px solid transparent; border-right:9px solid transparent; border-top:10px solid ${_light};"></span></div>
    // `;
...
let sellDotOverlays = []; // 줌레벨 4 점표시 오버레이 추적 배열
...
function createClusterer(estateType) {
    ...
    kakao.maps.event.addListener(clusterer, "clustered", function (clusters) {
        clusters.forEach(function (cluster) {
            ...
            const estateType = markers[0].estate_type; // 첫 번째 매물의 estate_type 사용 (색상 기준)
            const subEstate  = markers[0].sub_estate;  // 세부 매물종류 (표시명 분기용)
            const saleType = markers[0].sale_type; // 첫 번째 매물의 sale_type 사용
            const clusterCount = markers.length; // 클러스터에 포함된 매물의 개수

            // 매물종류별 색상 (estateType 기준 — displayName 아님)
            const clusterColors = getEstateTypeColors(estateType);
            const clusterBase  = clusterColors.base;
            const clusterLight = clusterColors.light;

            // sub_estate 반영 표시명
            const displayName = getEstateDisplayName(estateType, subEstate);

            // 뱃지 + 종류명 공통 상단 행 (그라데이션: 연한색→기본색)
            const badgeHtml = `
                <li style="background:linear-gradient(to right, ${clusterLight}, ${clusterBase}); padding:4px 7px; display:flex; align-items:center; justify-content:center; gap:3px; border-bottom:1px solid ${clusterBase}; white-space:nowrap;">
                    <span style="display:inline-flex; align-items:center; justify-content:center; width:15px; height:15px; border-radius:50%; background:${clusterBase}; font-size:8px; font-weight:800; color:#fff; flex-shrink:0; line-height:1; border:1px solid rgba(255,255,255,0.6);">매</span>
                    <span style="font-size:10px; font-weight:700; color:#222;">${displayName}</span>
                </li>`;
            ...
            if (clusterCount === 1) {
                ...
                switch (m.sale_type) {
                    case "임대": case "전세":
                        priceHtml = formatPrice(m.rent_price, "only-uk", true); break;
                    case "매매": case "교환":
                        priceHtml = formatPrice(m.sale_price, "only-uk", true); break;
                    default:
                        priceHtml = formatPrice(m.sale_price, "only-uk", true); break;
                }
                const rawArea = (estateType === "토지") ? m.plat_area : m.tot_area;
                const areaHtml = rawArea ? formatArea(rawArea) : "-";
                bodyHtml = `
                    <li style="background:#fff; padding:4px 8px;">
                        <p style="font-size:12px; font-weight:700; color:#222; line-height:13px; margin:0;">${priceHtml}</p>
                    </li>
                    <li class="sell-area-toggle" data-raw-area="${rawArea || ''}" style="background:${clusterBase}; padding:3px 8px; color:#fff; font-size:12px; cursor:pointer;" title="클릭하여 단위 변환">
                        ${areaHtml}
                    </li>`;
            } else {
                // ── 복수 클러스터: 건수 표시 ──
                bodyHtml = `
                    <li style="background:${clusterBase}; padding:5px 8px; color:#fff; font-size:14px; font-weight:700;">
                        ${clusterCount}건
                    </li>`;
            }
            ...
            const customClusterContent = `
                <div style="display:inline-flex; flex-direction:column; align-items:center;">
                    <ul style="min-width:62px; border-radius:10px; border:2px solid #dc3545; overflow:hidden; list-style:none; margin:0; padding:0; text-align:center;">
                        ${badgeHtml}
                        ${bodyHtml}
                    </ul>
                    <div style="width:100%; text-align:center; line-height:0; margin:0; padding:0;">
                        <span style="display:inline-block; width:0; height:0; border-left:9px solid transparent; border-right:9px solid transparent; border-top:10px solid ${clusterBase};"></span>
                    </div>
                </div>
            `;
            ...
            // 면적 단위 토글 (단독 마커일 때만 적용, 클릭 시 평 ↔ ㎡)
            const areaToggleEl = overlay.querySelector(".sell-area-toggle");
            ...
            // 삼각형이 UL 아래 중앙에 오도록 flex column 정렬
            overlay.style.display = "flex";
            overlay.style.flexDirection = "column";
            overlay.style.alignItems = "center";
            overlay.style.justifyContent = "center";
            // overlay.style.transform = "scale(0.8)"; // 풀마커는 원본 크기 유지 (실거래가와 동일)
