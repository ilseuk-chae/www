...
async function saleRegist(fileList, estateNo) {
    ...
    callApiFormData("POST", "/front/back/sale/sale_modify_v2.php", formData)
        .then((response) => {
            if (!response) {
...
async function saleInfo(estateNo) {
    ...
    const url = "/front/back/sale/sale_detail_v2.php";
    const dataObj = {
        ...user,
        estate_no: estateNo,
    };
...
async function detailBind(data) {
    ...
    // sub_estate 복원 (estate_type change 이벤트로 드롭다운 생성 후 값 설정)
    if (data.sub_estate) {
