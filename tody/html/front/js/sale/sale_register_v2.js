...
async function initSelect() {
    ...
    try {
        ...
        // subEstateMap 구성 (sub 항목이 있는 타입만)
        ESTATE_TYPE.forEach(function (item) {
            if (item.sub && item.sub.length > 0) {
                subEstateMap[item.type_code] = item.sub;
            }
        });
        ...
        // sub_estate 드롭다운 이벤트 바인딩
        $("#estate_type").off("change.subEstate").on("change.subEstate", function () {
            updateSubEstateOptions($(this).val());
        });
    } catch (error) {
...
function updateSubEstateOptions(val) {
    const SUB_ESTATE_TYPES = ['1001', '1006', '1007'];
    if (SUB_ESTATE_TYPES.includes(val) && subEstateMap[val]) {
        $("#sub_estate").empty().append('<option value="">(세부 매물종류)선택하세요</option>');
        subEstateMap[val].forEach(function (s) {
            $("#sub_estate").append(`<option value="${s.type_code}">${s.type_name}</option>`);
        });
        $("#sub_estate_wrap").show();
    } else {
        $("#sub_estate").empty().val("");
        $("#sub_estate_wrap").hide();
    }
}
...
function initEvents() {
    ...
    // $("#address_primary").on("input", function (e) {
    ...
    $("#sale_type").on("change", function () {
        ...
        // 임대일 때
        //if (value === "002") {
        if (value === "003") {  // 월세일때(2025-08 -11 거래종류 변경)
...
function initImageEvents() {
    ...
    async function handleImageDelete(fileList, element) {
        ...
        const fileId = slide.attr("data-id");
...
async function BuildingDetail(pnu) {
    ...
    if (!brTitleInfo || brTitleInfo.length === 0) {
        sweetAlertMessage("건축물대장 정보가 없습니다.", "", "i");
        return;
    }
    ...
    if (Array.isArray(brTitleInfo)) {
        ...
        brTitleInfo.sort(function (a, b) {
            ...
            const aSortKey = String(a.mainPurpsCdNm); // mainPurpsCdNm을 정렬 키로 사용
            const bSortKey = String(b.mainPurpsCdNm);
...
function createBuildingOptions(brTitleInfo, brRecapTitleInfo) {
    ...
    $.each(brTitleInfo, function (index, building) {
        const $option = $("<option>")
            ...
            .text(`${building.dongNm + " "}${building.bldNm + " "}${building.mainAtchGbCdNm}`); // 표시할 텍스트
        if (index === 0) {
...
function initModal() {
    // 모달 - 확인
    initializeModal("#modalConfirm", "/front/assets/lottie/save.json");
    // 모달 - 실패
        ...
        form.addEventListener(
            ...
            async function (event) {
                ...
                if (!isFormValid) {
                    ...
                    saleRegist(fileManager.getFileList()); // 여기서 getFileList로 fileList를 가져옴
                }
 ...
 * 저장 함수
 */
async function saleRegist(fileList) {
    ...
    // 대표 이미지 정보 추가
    formData.append("representativeImages", JSON.stringify(selectedRepresentativeImages)); // 대표 이미지 정보 배열을 JSON으로 직렬화하여 전송
    ...
    // estate_type이 "1007"일 경우 factoryData를 병합
    if (stringData.estate_type === "1007") {
        dataObj = {
            ...dataObj,
    ...
    callApiFormData("POST", "/front/back/sale/sale_regist_v2.php", formData)
        .then((response) => {
            if (!response) {
...
function getStringData() {
    return {
        ...
        sub_estate: encodeURIComponent(($("#sub_estate").val() || "").trim()),
