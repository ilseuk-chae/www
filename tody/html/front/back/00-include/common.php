...
function generateAuthData($conn, $userNo)
{
    ...
    $resultArray['userNo'] = hash('sha256', $userNo);
    $resultArray['contNo'] = $contMax;
    $resultArray['contToken'] = $contToken;
    $resultArray['userToken'] = $userToken;
...
function get_bound_query($query, $params)
{
    foreach ($params as &$value) {
        // NULL인 경우 처리
        if (is_null($value)) {
            $value = "NULL"; // 쿼리에서 NULL로 처리
        } else {
            // 값이 숫자인 경우 따옴표 없이, 그렇지 않은 경우 따옴표로 감싸줌
 ...
 * @param string $uuidFg 저장할 파일 이름을 uuid 처리할 것인지 유무(Y / N)
 * @return array 업로드 결과를 나타내는 배열 (성공 또는 실패 메시지와 파일 경로)
 */
function upload_file_one($file, $filePath, $uuidFg = 'N')
{
    $result_array = array();
    // 사이즈 체크
    if ($file['size'] <= 0) {
...
function makeApiRequest($url, $queryParams) {
    ...
    // Content-Type 헤더로 응답의 형식 파악
    $contentType = curl_getinfo($ch, CURLINFO_CONTENT_TYPE);
    curl_close($ch);

    // 응답이 JSON 형식일 경우 처리
    if (strpos($contentType, 'application/json') !== false) {
        return json_decode($response, true); // JSON을 PHP 배열로 변환
    }

    // 응답이 XML 형식일 경우 처리
    if (strpos($contentType, 'application/xml') !== false || strpos($contentType, 'text/xml') !== false) {
        $xml = simplexml_load_string($response); // XML을 SimpleXMLElement로 변환
        if ($xml === false) {
            return false; // XML 파싱 실패 시 false 반환
        }
