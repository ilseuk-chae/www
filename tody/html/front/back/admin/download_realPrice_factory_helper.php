<?php
// download_realPrice_factory_helper.php
...
define('REALPRICE_FACTORY_TABLE', 'realPrice_factory_' . TARGET_REGION_PREFIX); // DB 저장 테이블명 commercial
...
while ($start_date <= $end_date) {
    ...
    foreach ($bjd_list as $bjd_cd) {
        ...
        if ($row_count > 0) {
            //fwrite(STDERR, "WARN: (공장/창고)이미 존재하는 날짜입니다. bjd_cd: $bjd_cd, 날짜: $ymd" . PHP_EOL); // 헬퍼는 로그를 상세히 남기지 않음
            log_to_db($historyId, "[bjd_cd: $bjd_cd] (공장/창고)이미 존재하는 날짜($ymd)입니다: ", $conn, 'INFO');
            continue;
        }
        ...
        try {
            ...
            if ($response) {
                ...
                $processed = 0;
                 ...
                 if (isset($json['body']['items']['item'])) {
                    ...
                    foreach ($items as $item) {
                        ...
                        if(processAndSaveItem($conn, $item, REALPRICE_FACTORY_TABLE)){ // 함수 이름 변경
                            $insertedCount++;
                        }
                    }
                } else {
                ...
                $insert_sql = "INSERT INTO realprice_down_his (type, bjdCd, date, count) VALUES ('factory', ?, ?, ?)";
                ...
                fwrite(STDERR, "FAILED: bjd_cd: $bjd_cd, 수집월: $ymd -> No response from API (factory)." . PHP_EOL);
                $overallSuccess = false;
                break 2;
            }
        } catch (Exception $e) {
            fwrite(STDERR, "FAILED: bjd_cd: $bjd_cd, 수집월: $ymd -> Error (factory): {$e->getMessage()}" . PHP_EOL);
            $overallSuccess = false;
            break 2;
        }
    }
...
if ($overallSuccess) {
    ...
    echo json_encode([
        'status' => 'SUCCESS',
        'message' => '국토교통부 실거래가(공장/창고) 가져오기 완료.',
    ...
    echo json_encode([
        'status' => 'FAILED',
        'message' => '국토교통부 실거래가(공장/창고) 가져오기 실패.',
...
/**
 * 거래 정보를 기준으로 Redis에 저장된 공장/창고 특성(PNU)을 찾아 결합한다.
 * $item: 실거래가 정보 배열
 */
...
/**
 * API 항목을 정규화하여 공장/창고 실거래 테이블에 저장한다.
 */
function processAndSaveItem($conn, $item)
{
    ...
    $sql = "INSERT IGNORE INTO " . REALPRICE_FACTORY_TABLE . " (" . implode(', ', $columns) . ") VALUES ($placeholders)";
