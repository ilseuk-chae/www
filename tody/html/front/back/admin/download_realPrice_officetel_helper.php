<?php
// download_realPrice_officetel_helper.php
...
$sql = "SELECT CONCAT(sido_cd, sgg_cd) AS bjd_cd FROM bjd_master WHERE depth = 2 AND LEFT(region_cd, 2) = '" . TARGET_REGION_PREFIX . "' ORDER BY bjd_cd ASC;";
...
while ($start_date <= $end_date) {
    ...
    foreach ($bjd_list as $bjd_cd) {
        ...
        if ($row_count > 0) {
            //fwrite(STDERR, "WARN: (오피스텔)이미 존재하는 날짜입니다. bjd_cd: $bjd_cd, 날짜: $ymd" . PHP_EOL); // 헬퍼는 로그를 상세히 남기지 않음
            log_to_db($historyId, "[bjd_cd: $bjd_cd] (오피스텔)이미 존재하는 날짜($ymd)입니다: ", $conn, 'INFO');
            continue;
        }

        try {
            //$url = "http://apis.data.go.kr/1613000/RTMSDataSvcOffiTrade/getRTMSDataSvcOffiTrade"; // 오피스텔 API 엔드포인트
                ...
                if (isset($json['body']['items']['item'])) {
                    ...
                    if (isset($items[0])) {
                        foreach ($items as $item) {
                            processAndSaveItem($conn, $item,REALPRICE_OFFICETEL_TABLE); // 함수 이름 변경
                        }
                    } else {
                        processAndSaveItem($conn, $items,REALPRICE_OFFICETEL_TABLE); // 함수 이름 변경
                    }
                    ...
                    $insert_stmt->execute();
                    $insert_stmt->close();
                    log_to_db($historyId, "INFO: bjd_cd: $bjd_cd, 수집월: $ymd -> No data found(오피스텔)", $conn, 'INFO');
                }
            } else {
                fwrite(STDERR, "FAILED: bjd_cd: $bjd_cd, 수집월: $ymd -> No response from API (Officetel)." . PHP_EOL);
                $overallSuccess = false;
                break 2;
            }
        } catch (Exception $e) {
            fwrite(STDERR, "FAILED: bjd_cd: $bjd_cd, 수집월: $ymd -> Error (Officetel): {$e->getMessage()}" . PHP_EOL);
            $overallSuccess = false;
            break 2;
        }
    }
...
if ($overallSuccess) {
    ...
    echo json_encode([
        'status' => 'SUCCESS',
        'message' => '국토교통부 실거래가(오피스텔) 가져오기 완료.',
    ...
    echo json_encode([
        'status' => 'FAILED',
        'message' => '국토교통부 실거래가(오피스텔) 가져오기 실패.',
...
function processAndSaveItem($conn, $item)
{
    ...
    $sql = "INSERT IGNORE INTO " . REALPRICE_OFFICETEL_TABLE . " (" . implode(', ', $columns) . ") VALUES ($placeholders)";
