<?php
// download_realPrice_commercial_helper.php
...
// 기존 파일의 상수 정의를 따릅니다.
define('PNU_SCORE_THRESHOLD', 65);  //70==>65
define('AREA_TOLERANCE', 20); //5->20 상업/업무토지는 면적 허용오차를 20으로 설정
define('TARGET_REGION_PREFIX', $targetRegionPrefix);
define('REALPRICE_COMMERCIAL_TABLE', 'realPrice_commercial_' . TARGET_REGION_PREFIX); // DB 저장 테이블명 commercial
...
$policy = new CommercialScorePolicy();
...
while ($start_date <= $end_date) {
    ...
    if (check_cancellation($historyId, $conn)) {
        throw new Exception("Sido {". TARGET_REGION_PREFIX ."} 작업 도중 사용자 중단 요청 감지. 작업 중단.", 500);
    }
    ...
    foreach ($bjd_list as $bjd_cd) {
        ...
        */
        ...
        if ($row_count > 0) {
            //fwrite(STDERR, "WARN: (상업/업무)이미 존재하는 날짜입니다. bjd_cd: $bjd_cd, 날짜: $ymd" . PHP_EOL); // 헬퍼는 로그를 상세히 남기지 않음
            log_to_db($historyId, "[bjd_cd: $bjd_cd] (상업/업무)이미 존재하는 날짜($ymd)입니다: ", $conn, 'INFO');
            ...
            if ($response) {
                ...
                if (isset($json['body']['items']['item'])) {
                    ...
                    foreach ($items as $item) {
                        ...
                        if(processAndSaveItem($conn, $item, REALPRICE_COMMERCIAL_TABLE)){ // 함수 이름 변경
                            $insertedCount++;
                        }
                    }
                } else {
                ...
                fwrite(STDERR, "FAILED: bjd_cd: $bjd_cd, 수집월: $ymd -> No response from API (commercial)." . PHP_EOL);
                $overallSuccess = false;
                break 2;
            }
        } catch (Exception $e) {
            fwrite(STDERR, "FAILED: bjd_cd: $bjd_cd, 수집월: $ymd -> Error (commercial): {$e->getMessage()}" . PHP_EOL);
            $overallSuccess = false;
            break 2;
        }
    }
...
if ($overallSuccess) {
    ...
    echo json_encode([
        'status' => 'SUCCESS',
        'message' => '국토교통부 실거래가(상업/업무) 가져오기 완료.',
    ...
    echo json_encode([
        'status' => 'FAILED',
        'message' => '국토교통부 실거래가(상업/업무) 가져오기 실패.',
...
/**
 * 국토교통부 상업/업무 실거래 API를 모두 호출하여 전체 페이지를 수집한다.
 */
...
/**
 * 거래 정보를 기준으로 Redis에 저장된 상업/업무토지 특성(PNU)을 찾아 결합한다.
 * $item: 실거래가 정보 배열
 */
function findPnuInBuildingRedis(mysqli $conn, $item, $redis, $policy, $resolver, $historyId)
{
    ...
    if ($results === false || !is_array($results)) {
        error_log("[ERROR] Redis pipeline exec 실패 (상업용 조회). sggCd={$sggCd}, dongNm={$umdNm}");
        return null;
    }
    ...
    if (empty($candidates)) {
     ...
     $context = [
        ...
        'buildingAr'  => (float)($item['buildingAr'] ?? 0),  //건물
        'buildingType'=> $item['buildingType'] ?? '',
        'floor'        => (int)($item['floor'] ?? 0),   // 🔥 추가
        'sggCd'       => $sggCd,
        'umdCd'       => $umdCd,
    ];
    ...
    foreach ($candidates as &$c) {  
        ...
        $c['buildingAr'] = $titleTot > 0 ? $titleTot : $totalTot;
        ...
        /*
        if ($context['buildingType'] === '일반' && $c['plottageAr'] == 0) {
            continue;
        }
        */

        // 일반건물: 대지면적 없음 강필터: 층수 + 건물용도
        if ($context['buildingType'] === '집합') {
            if (!empty($context['floor']) &&
                $context['floor'] > (int)$c['grndFlrCnt']) {
                continue;
            }
        }
 ...
 */
function getUmdCdByName($sggCd, $umdNm) //토지와동일
{
    static $cache = [];
 ...
 */
function resolveUmdQueryInfo($umdNm) //토지와동일
{
    if ($umdNm === null) {
...
/**
 * 상업/업무토지 거래 항목을 정규화해 임시 테이블에 저장한다.
 */
function processAndSaveItem($conn, $item)
{
    ...
    $sql = "INSERT IGNORE INTO " . REALPRICE_COMMERCIAL_TABLE . " (" . implode(', ', $columns) . ") VALUES ($placeholders)";
...
/**
 * 상업/업무토지 관련 문자열을 trim한 형태로 통일한다.
 */
