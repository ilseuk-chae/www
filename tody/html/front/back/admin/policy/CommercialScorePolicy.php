<?php
// /admin/policy/CommercialScorePolicy.php

require_once __DIR__ . '/ScorePolicyInterface.php';

class CommercialScorePolicy implements ScorePolicyInterface
{
    public function score(array $c, array $ctx): float  //ctx: (실거래가)검색조건, c: (Redis)후보지
    {
        $score = 0;

        $type  = $this->resolveType($ctx);

        //$candidateArea = $this->getBestTotArea($c);  // 후보지 건물면적
        ...
        if ($type === 'complex') {
            ...
            // 집합: 건물(연)면적 + 층 중심
            $score += $this->scoreBuildingArea($candidateBldArea, $targetBldArea) * 1.2;
            $score += $this->scoreFloor($c, $ctx) * 1.5;
            $score += $this->scoreUse($c['mainPurpsCdNm'] ?? '', $ctx['buildingUse'] ?? '');

        } elseif ($type === 'general') {

            // 일반: 대지 중심 total_platArea: 후보지(redis) 대지면적, plottageAr: 실거래가 대지면적
    ...
    public function tolerance(array $ctx): float
    {
        ...
        if ($type === 'complex') {
            return max(20, $area * 0.05); // 집합 더 엄격
        }
    
        if ($type === 'general') {
            return max(30, $area * 0.08);
        }
    ...
    private function scoreBuildingArea($candidateAr, $targetAr)
    {
        ...
        if ($diffRatio >= 0.25) return 0;

        // 🔥 0~5% 구간: 75 → 60 선형 감소
        if ($diffRatio <= 0.05) {
            // diffRatio = 0 → 75
            // diffRatio = 0.05 → 60
            return 75 - ($diffRatio / 0.05) * 15;
        }

        // 🔥 5~25% 구간: 60 → 0 선형 감소
        return 60 * (1 - ($diffRatio - 0.05) / 0.20);
    }
    
    /* 대지면적은 건물면적보다 중요도가 낮으므로 최대 15점 */
    /* candidateAr: 후보지(redis) 대지면적, targetAr: 실거래가 대지면적 */
    /* 대지면적 점수 (최대 15점) */
    private function scorePlottageArea($candidateAr, $targetAr)
    {
        ...
        // 0~5% : 15 → 13.5
        if ($diffRatio <= 0.05) {
            return 15 - ($diffRatio / 0.05) * 1.5;
        }

        // 5~10% : 13.5 → 12
        if ($diffRatio <= 0.10) {
            return 13.5 - (($diffRatio - 0.05) / 0.05) * 1.5;
        }

        // 10~20% : 12 → 9
        if ($diffRatio <= 0.20) {
            return 12 - (($diffRatio - 0.10) / 0.10) * 3;
        }

        // 20~30% : 9 → 6
        if ($diffRatio <= 0.30) {
            return 9 - (($diffRatio - 0.20) / 0.10) * 3;
        }

        // 30~40% : 6 → 3
        if ($diffRatio <= 0.40) {
            return 6 - (($diffRatio - 0.30) / 0.10) * 3;
        }

        // 40~50% : 3 → 0
        return 3 - (($diffRatio - 0.40) / 0.10) * 3;
    }
/*
    private function scorePlottageArea($candidateAr, $targetAr)  
    {
