...
try {
    ...
    #######################################################
    # 1. 매물 등록 (estate_listings_v2)
    #######################################################
    $sql =
        "INSERT INTO estate_listings_v2 (
            pnu, postal_code, address_jibun, address_road, address_detail,
            estate_type, sub_estate, sale_type, exchange_fg, urgent_sale_fg,
    ...
    $types = 'ssssssssss' . 'ssss' . 'ssss' . 'sss' . 'sssss' . 'iiii' . 'ddii' . 'ddd' . 'ddds' . 'ddi';
...
function handleMultipleFileUploads($files, $user_no, $estate_no, $representativeImages)
{
    ...
    if (!empty($values)) {
        ...
        if (empty($representativeImages)) {
            $representativeImages = count($insertedFileIds) === 1 ? [0] : [0, 1];
        }
