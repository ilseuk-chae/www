...
$proposal_type = isset($_POST['proposal_type']) ? urldecode($_POST['proposal_type']) : '';
...
$validations = [
    ...
    ['value' => $proposal_type, 'type' => 'string', 'message' => '제안 종류를 확인해주세요.'],
];
...
try {
    ...
    ##################### 1.삭제 처리 #####################
    $sql =
        "INSERT INTO proposal_master (
            title, content, proposal_type, reg_no
        ) VALUES (
            ?, ?, ?, ?
        );
        ";
    ...
    // 조건 추가
    $params = [$title, $content, $proposal_type, $user_no];
    $types = 'sssi';
