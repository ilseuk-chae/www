...
try {
    ...
    # 3. 기존 알림 구역 리스트 제거
    #######################################################
    $sql3 = "DELETE FROM user_notification_list_v2 WHERE user_no = ?";
    ...
    if ($user_role === "002") {
        ...
        if (!empty($find_array)) {
            ...
            $sql4 = "INSERT INTO user_notification_list_v2 (user_no, noti_type, estate_type, sido_cd, sgg_cd) VALUES ";
            $values = [];
            $params = [];
            $types = '';  // types 문자열 초기화
        ...
        if (!empty($put_array)) {
            // 다중 행 삽입을 위한 SQL 쿼리 생성
            $sql5 = "INSERT INTO user_notification_list_v2 (user_no, noti_type, estate_type, sido_cd, sgg_cd) VALUES ";
            $values = [];
            $params = [];
            $types = '';  // user_no의 타입
