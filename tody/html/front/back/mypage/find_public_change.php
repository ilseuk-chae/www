...
$wanted_no = isset($_POST['wanted_no']) ? urldecode($_POST['wanted_no']) : '';
...
$valid = validateInput($wanted_no, 'int', $errorMessage, array());
if ($valid == $errorMessage) {
    responseApi(400, $errorMessage, null);
    exit;
}
...
try {
    ...
    $sql =
        ...
        SET 
            public_fg = ?
        WHERE reg_no = (SELECT user_no FROM user_master WHERE user_no_hmac = ?) 
        AND idx = ?;
        ";

    // // 바인딩할 변수들
    // $params = array($rcvNo, $wanted_no);
    // // 실제 실행될 쿼리문
    ...
    mysqli_stmt_bind_param($stmt, "ssi", $public_fg, $userNo, $wanted_no);
