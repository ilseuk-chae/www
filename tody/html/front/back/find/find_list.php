...
//error_log("Received ALL POST data: " . print_r($_POST, true));
...
// 이제 isset($_POST[...]) 검사를 하더라도 $minPrice 등의 변수는 이미 정의되어 있습니다.
...
//$estate_type = isset($_POST['estateType']) ? urldecode($_POST['estateType']) : ''; // 'estate_type' -> 'estateType' --- 수정됨 ---
//$estate_type = isset($_POST['estateType']) ? urldecode($_POST['estateType']) : $estate_type;
...
//$sale_type = isset($_POST['saleType']) ? urldecode($_POST['saleType']) : $sale_type;     // 'sale_type' -> 'saleType'     --- 수정됨 ---
...
//$min_area = isse
// t($_POST['min_area']) ? urldecode($_POST['min_area']) : '';
...
//error_log("Read variables: minPrice=" . $minPrice . ", maxPrice=" . $maxPrice . ", minArea=" . $minArea . ", maxArea=" . $maxArea);
// 타입 확인도 중요합니다.
//error_log("Type of maxPrice: " . gettype($maxPrice));
// 디버깅용: 클라이언트로부터 받은 모든 POST 데이터 출력 (권장)
//error_log("Received ALL POST data: " . print_r($_POST, true));
// 읽어온 파라미터 값 확인 (배열 변수에 제대로 담겼는지)
//error_log("Server variables: sido=" . $sido . ", sgg=" . $sgg . ", estate_type=" . print_r($estate_type, true) . ", sale_type=" . print_r($sale_type, true) . ", min_price=" . $minPrice . ", max_price=" . $maxPrice . ", min_area=" . $minArea . ", max_area=" . $maxArea . ", sort=" . $sort . ", page=" . $page . ", items_per_page=" . $items_per_page);
...
try {
    ...
    //if ($min_price !== '') {
    //    $sql .= " AND a.min_price >= ?"; // 이제 여기에 '500000000' 같은 값이 담깁니다.
    //    $params[] = $min_price;
    //    $types .= 'i';
    //}
   ...
   if (!empty($params)) {
    ...
    // DataTables 표준 응답 형식에 맞게 JSON 응답의 최상위 키를 'responseData'에서 'data'로 변경
    $response_array = [
        'responseData' => $response_data,
        'total_records' => $total_records,
        'recordsFiltered' => $total_records, // F
    ...
    // responseApi($e->getCode(), $e->getMessage(), null); // 이 코드는 주석 처리하거나 제거
} finally {
    // 연결 종료
    if (isset($stmt))
