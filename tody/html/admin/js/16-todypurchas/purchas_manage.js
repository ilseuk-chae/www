...
function initializeDataTable() {
    purchasTable = new DataTable('#ajax-datatables', {
        ...
        columns: [
            { data: 'no',            title: 'No' },
            { data: 'type_label',    title: '신청구분' },
            { data: 'name',          title: '담당자' },
            { data: 'company',       title: '회사' },
            { data: 'phone',         title: '연락처' },
            { data: 'file_count',    title: '첨부', orderable: false },
...
function loadTableData(callback) {
    ...
    callApi('POST', '/admin/back/16-todypurchas/inquiry_list.php', params, 'noLoading')
        .then((res) => {
            ...
            const rows = res.responseData.map((row) => {
                const typeLabel = PURCHAS_TYPE_LABEL[row.type] || row.type;
                const statusLabel = PURCHAS_STATUS_LABEL[row.status] || row.status;
                const badgeCls = PURCHAS_STATUS_BADGE[row.status] || 'bg-light text-dark';
                ...
                const management = `
                    <div class="dropdown d-inline-block">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end">
                            <li>
                                <a href="/admin/views/purchas_manage/purchas_manage_detail.html?no=${row.no}" class="dropdown-item">
                ...
                return {
                    ...
                    file_count: row.file_count > 0
                        ? `<span class="badge bg-primary-subtle text-primary">${row.file_count}</span>`
                        : '-',
        ...
        .catch((err) => {
            console.error('구매대행 목록 로드 실패:', err);
            if (callback) callback({ data: [] });
        });
}
...
async function inquiryDelete(no) {
    ...
    const res = await callApi('POST', '/admin/back/16-todypurchas/inquiry_delete.php', params);
    if (!res) return;

    if (res.message === 'SUCCESS') {
        await sweetAlertForReturn('삭제 되었습니다.', '', 's');
        purchasTable.ajax.reload(null, false);
    } else {
