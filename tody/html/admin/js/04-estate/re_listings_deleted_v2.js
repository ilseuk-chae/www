...
$(document).ready(async function () {
    ...
    // [Event] 삭제취소 버튼 클릭
    $(document).on("click", ".delete-restore-btn", function () {
...
function loadTableData(table, callback) {
    ...
    callApi("POST", "/admin/back/04-estate/estate_list_deleted_v2.php", { ...adminUserInfo(), public_fg: filterValue }, "loading")
        .then((result) => {
            if (!result) {
                console.log("통신 실패!!!");
            ...
            const formattedData = responseData.map((item, index) => {
                ...
                const addressTotal = item.address_jibun || "";
                const descBtn = `<a class="link-dark link-body-emphasis link-offset-2 text-decoration-underline link-underline-opacity-25 link-underline-opacity-75-hover" href="/admin/views/re_manage/re_detail_deleted_v2.html?no=${item.estate_no}">${addressTotal}</a>`;
                ...
                return {
                    ...
                    <div class="dropdown d-inline-block">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end">
                            <li>
                                <a href="/admin/views/re_manage/re_detail_deleted_v2.html?no=${item.estate_no}" class="dropdown-item">
                                    <i class="ri-eye-fill align-bottom me-2 text-muted"></i>
                                    상세
                                </a>
                            </li>
                            <li>
                                <button data-no="${item.estate_no}" class="delete-restore-btn dropdown-item">
                                    <i class="ri-repeat-line align-bottom me-2 text-muted"></i>
                                    복구
                                </button>
                            </li>
                        </ul>
 ...
 * @returns
 */
async function estate_restore(rcvNo) {
    const confirm = await sweetConfirm("복구 하시겠습니까?", "", "w");
    if (!confirm) return;
    ...
    const result = await callApi("POST", "/admin/back/04-estate/estate_restore.php", dataObj);
    ...
    if (message === "SUCCESS") {
        ...
        const confirm = await sweetAlertForReturn("복구에 실패했습니다.", "", "e");
        if (!confirm) return;
    }
}
