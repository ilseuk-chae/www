 ...
 * @returns
 */
async function re_put_detail_info() {
    ...
    const result = await callApi("POST", "/admin/back/04-estate/put_info_v2.php", dataObj);
    ...
    const { latitude, longitude, public_fg, imageArray } = responseData;
...
async function put_delete(rcvNo) {
    ...
    if (message === "SUCCESS") {
        ...
        if (!confirm) return;
        location.href = "/admin/views/re_manage/re_put_v2.html";
    } else {
...
async function public_fg_change(val) {
    ...
    const langCode = localStorage.getItem("langCode") ?? "kr"; // 언어
    const adminInfo = adminUserInfo(); // 관리자 정보
    const put_no = getUrlParameter("no");
