...
function initializeDataTable() {
    let table = new DataTable("#ajax-datatables", {
        ...
        columns: [
            ...
            { data: "min_price", title: "가격대(최소)<br>(단위: 만)", searchable: false },
            { data: "max_price", title: "가격대(최대)<br>(단위: 만)", searchable: false },
...
function loadTableData(table, callback) {
    ...
    callApi("POST", "/admin/back/04-estate/wanted_list_v2.php", dataObj, "loading")
        .then((result) => {
            if (!result) {
                console.log("통신 실패!!!");
            ...
            const formattedData = responseData.map((item, index) => {
                ...
                return {
                    ...
                    <div class="dropdown d-inline-block">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end">
                            <li>
                                <a href="/admin/views/re_manage/re_wanted_detail_v2.html?no=${item.wanted_no}" class="dropdown-item">
...
// =============================================================================
/**
 * 매물종류 가져오는 함수 (V2: estate_type_get_v2.php 사용)
 * @returns
 */
async function estate_type_get() {
    ...
    const result = await callApi("POST", "/admin/back/04-estate/estate_type_get_v2.php", dataObj);
