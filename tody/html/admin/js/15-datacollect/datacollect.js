...
$(document).ready(async function () {
    ...
    const elSido = document.querySelector('.sido-checkboxes-group');
    const elBtn = document.querySelector('.target-button-section');
    const elStatus = document.querySelector('.target-status-section');
    const elHistory = document.querySelector('.target-history-section');
    const elContent = document.querySelector('.target-contents-section');
    const contentName = $('#ContentsTitle');           // ===>>> jQuery 객체 <<<===
    ...
    async function updateContentAndButton(dataType) {
        ...
        switch (dataType) {
            case 'collect_bjd':
                descriptionContent  = '<p>법정동 코드를 수집, 수정 후 저장합니다</p>';
                buttonText = '법정동  데이터 수집하기 시작';
                logHeading.textContent = '[법정동  데이터 수집하기 로그]';
                //showDateSelection = true;
                //showresetSelect = true;
                //showstartDateSelection = false;
                //showendDateSelection = false;
                //showbaseDateSelection = false;
                elSido.style.display = ''; // 시도 체크박스 그룹 보이기 
                elBtn.style.display = '';  // 시작 버튼 섹션 그룹 보이기
                elStatus.style.display = ''; // 상태 정보 표시 영역 보이기
                elHistory.style.display = ''; // 이력 정보 표시 영역 보이기
                elContent.style.display = ''; // 콘텐츠 정보 표시 영역 보이기
                contentName.text('법정동  데이터 수집방법'); // 콘텐츠 제목 업데이트
                break;
            case 'cadastral_map':
                descriptionContent  = '<p>(수동)연속지적도형 정보를 수집하는 방법을 설명합니다.</p>';
                buttonText = '';
                logHeading.textContent = '';
                //showDateSelection = true;
                //showresetSelect = true;
                //showstartDateSelection = false;
                //showendDateSelection = false;
                //showbaseDateSelection = false;
                elSido.style.display = 'none'; // 시도 체크박스 그룹 보이기 
                elBtn.style.display = 'none';  // 시작 버튼 섹션 그룹 보이기
                elStatus.style.display = 'none'; // 상태 정보 표시 영역 보이기
                elHistory.style.display = 'none'; // 이력 정보 표시 영역 보이기
                elContent.style.display = ''; // 콘텐츠 정보 표시 영역 보이기
                contentName.text('연속지적도형 정보(WMS/WFS) 수집방법'); // 콘텐츠 제목 업데이트
                break;
            case 'characteristics_info':
                descriptionContent  = '<p>(수동)토지특성 정보를 수집하는 방법을 설명합니다.</p>';
                buttonText = '';
                logHeading.textContent = '';
                //showDateSelection = true;
                //showresetSelect = true;
                ...
                elHistory.style.display = 'none'; // 이력 정보 표시 영역 보이기
                elContent.style.display = ''; // 콘텐츠 정보 표시 영역 보이기
                contentName.text('토지특성 정보 수집방법'); // 콘텐츠 제목 업데이트
                break;  
            case 'boundary_info':
                descriptionContent  = '<p>(수동)경계데이터 정보(2곳)를 수집하는 방법을 설명합니다.</p>';
                buttonText = '';
                logHeading.textContent = '';
                //showDateSelection = true;
                //showresetSelect = true;
                ...
                elHistory.style.display = 'none'; // 이력 정보 표시 영역 보이기
                elContent.style.display = ''; // 콘텐츠 정보 표시 영역 보이기
                contentName.text('경계 데이터 수집방법'); // 콘텐츠 제목 업데이트
                break;
            default:
        ...
        loadContent(dataType); // 탭별 콘텐츠 로드 함수 호출 (필요 시 구현)
    ...
    */
    ...
    function initEvents() {
        ...
        $(uploadStartBtn_main).on("click", async function () {
            ...
            switch (currentDataType) {
                case 'collect_bjd':
                    apiUrl = '/admin/back/15-datacollect/start_datacollect_bjd.php';
    ...
    function historyTablerefresh(dataType) {
        if (dataType === 'collect_bjd') {
            initHistoryViewCollectBjd(10, 0);
        }       
    }
    ...
    function loadContent(taskType) {
        let file = '';
        switch (taskType) {
            case 'collect_bjd':
                file = "/admin/views/datacollect_manage/collect_bjd.html";
                break;
            case 'cadastral_map':
                file = "/admin/views/datacollect_manage/cadastral_map.html";
                break;
            case 'characteristics_info':
                file = "/admin/views/datacollect_manage/characteristics_info.html";
                break;
            case 'boundary_info':
                file = "/admin/views/datacollect_manage/boundary_info.html";
                break;
        }
        fetch(file)
          .then(response => response.text())
          .then(html => {
            document.getElementById('sub_content').innerHTML = html;
          })
          .catch(err => {
            document.getElementById('sub_content').innerHTML = '<p style="color:red;">파일을 불러올 수 없습니다.</p>';
            console.error(err);
          });
    }
    // ===============================================================
    ...
    async function initHistoryViewCollectBjd(limit, offset) {
        await getAndRenderHistory('collect_bjd', limit, offset);
    }
});