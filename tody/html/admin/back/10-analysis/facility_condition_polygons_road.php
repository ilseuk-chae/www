...
try {
    ...
    // API 데이터 가져오기
    $roadPolygon = getaRoadPolygon($domain, $apiKey, $pnu);

    if (!isset($roadPolygon['response']['status'])) {
        throw new Exception('API 호출 실패', 500);
    } 
    if ($roadPolygon['response']['status'] !== 'OK' && $roadPolygon['response']['status'] !== 'NOT_FOUND') {
        throw new Exception('API 호출 실패', 500);
    }
    if ($roadPolygon['response']['status'] === 'NOT_FOUND') {
        responseApi(200, 'NO POLYGON DATA FOUND', []);
        exit;
    } 

    $features = $roadPolygon['response']['result']['featureCollection']['features'];
    if (empty($features)) {
        responseApi(200, 'NO POLYGON DATA FOUND', []);
        exit;
    }
...
try {
    ...
    // 3. API 응답 가져오기
    $ariculturalPolygon = getaRoadPolygon($domain, $apiKey, $pnu);
