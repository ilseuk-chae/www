...
try {
    ...
    // echo json_encode($facilitys);exit;
    ...
    $mainSql = 
        ...
        JOIN (
            ...
            WHERE pnu LIKE :pnuParam AND "lndcgrCode" = :code
    ...
    // $mainSql = 
    //     'WITH temp AS (
    //         SELECT wkt, pnu FROM temp_facilitys
    //     ),
    //     buffer_table AS (
    //         SELECT
    //             ST_SetSRID(wkt, 4326) AS wkt,
    //             pnu
    //         FROM tody.land_characteristics_polygon_41
    ...
    //     WHERE EXISTS (
    //         SELECT 1
    //         FROM tody.land_characteristics_polygon_41 AS buffer_table
    //         WHERE buffer_table.pnu LIKE :pnuParam
    //         AND buffer_table."lndcgrCode" = :code
    //         AND ST_Intersects(
    //             ST_Buffer(temp.wkt, :bufferDistance),
...
try {
    ...
    //     $decodedInput = gzdecode($input);
    //     if ($decodedInput === false) {
    //         throw new Exception("압축 해제 실패");
    //     }
    //     $postData = json_decode($decodedInput, true);
    // } else {
    //     // 압축되지 않은 경우 JSON으로 바로 처리
    //     $postData = json_decode($input, true);
    // }

    // if ($postData === null) {
    //     throw new Exception("JSON 데이터 파싱 실패");
    // }

    // // 데이터 변수 추출
    // $pnu = $postData['pnu'] ?? null;
    // $page = (int)($postData['page'] ?? 0);
    // $limit = 10; // 한 번에 가져올 레코드 개수
    // $offset = $page * $limit; // 페이지 오프셋 계산
    // $facilitys = $postData['facilitys'] ?? []; // 분석 시설 배열
    // $condition = $postData['condition'] ?? []; // 분석 조건 배열
    // $radius = $condition['radius'] ?? null; // 조건 반경
    // $code = $condition['code'] ?? null; // 조건 코드

    // JSON 입력 데이터 디코딩
    // $inputData = json_decode(file_get_contents("php://input"), true);
    // $pnu = $inputData['pnu'] ?? null;
    // $page = (int)($inputData['page'] ?? 0);
    // $facilitys = $inputData['facilitys'] ?? [];
    // $condition = $inputData['condition'] ?? [];
    // $radius = $condition['radius'] ?? null;
