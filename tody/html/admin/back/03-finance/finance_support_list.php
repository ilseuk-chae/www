...
try {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        // 회사가 아직 배열에 없다면 추가
        if (!isset($supports[$support_no])) {
            $supports[$support_no] = array(
                'support_no' => $row['support_no'],
                'address_road' => $row['address_road'],
                'address_jibun' => $row['address_jibun'],
                'address_detail' => $row['address_detail'],
                'loan_desired_amount' => $row['loan_desired_amount'],
        ...
        // 지점 정보 추가
        if ($row['b_name']) {
            $supports[$support_no]['branches'][] = array(
                'c_name' => $row['c_name'],
                'b_name' => $row['b_name'],
        ...
        $response_data = array_values($supports);
    }
