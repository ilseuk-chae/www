...
try {
    ...
    $wanted_table = $v2_mode ? 'wanted_listings_v2' : 'wanted_listings';
    ...
    if (!empty($pnu)) {
        ...
        $sql =
            "SELECT
                ...
                bm.locatadd_nm AS address_jibun

            FROM history_recent_view AS hrv

            INNER JOIN {$wanted_table} AS wl
            ON wl.idx = hrv.board_no
            AND wl.active_fg = 'Y'

            INNER JOIN (SELECT sido_cd, sgg_cd, MIN(locatadd_nm) AS locatadd_nm FROM bjd_master GROUP BY sido_cd, sgg_cd) AS bm
            ON bm.sido_cd = wl.sido_cd
            AND bm.sgg_cd = wl.sgg_cd

            WHERE hrv.view_type = 'find'
            AND CONCAT(wl.sido_cd, wl.sgg_cd) LIKE CONCAT(?, '%') -- sido_cd와 sgg_cd를 결합하여 pnu로 시작하는 값 찾기
            AND hrv.reg_date >= DATE_SUB(NOW(), INTERVAL ? MONTH)

            GROUP BY hrv.board_no
            ORDER BY count DESC
            LIMIT ? OFFSET ?";
