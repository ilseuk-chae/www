    ...
    if (!empty($pnu)) {
        ...
        $sql =
            "SELECT 
                ...
                latitude,
                longitude,
                COUNT(*) AS pnu_count -- 동일한 pnu를 가진 데이터의 수
                
            FROM history_recent_realPrice

            WHERE pnu LIKE CONCAT(?, '%')
            AND reg_date >= DATE_SUB(NOW(), INTERVAL ? MONTH)

            GROUP BY pnu, jibun_address, latitude, longitude
        ...
        $sql =
            ...
            GROUP BY pnu, address_jibun, latitude, longitude
            ORDER BY reg_date DESC
            LIMIT ? OFFSET ?
            ";
