...
try {
    ##################### 1. 직군 테이블에서 제거 #####################
    $sql = "DELETE FROM relationship_job WHERE job_no = ?; ";
    ...
    mysqli_stmt_bind_param($stmt, "s", $jobNo);
    ...
    ##################### 1. 유저 직군 제거 #####################
    $sql2 = "DELETE FROM relationship WHERE job_no = ?; ";
    ...
    // 변수 바인딩 (s: string, i: integer 등)
    mysqli_stmt_bind_param($stmt2, "s", $jobNo);
